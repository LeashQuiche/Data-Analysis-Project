...
def kNearestNeighbor(normArr, testCase, k):
    ...
    for i in range(normArr.len):
        distArr[i] = NNC.findDistance(normArr.h[i], normArr.g[i], testCase[1], testCase[0])
...
def graphKNearestNeighbor(testCase, distArr, normArr):
    ...
    plt.plot(testCase[1], testCase[0], color[int(testCaseClass)] + 'x', label = "Test case", 
             markersize = 15)
    plt.plot(normArr.h[distArr], normArr.g[distArr], 'yo', label = "k Nearest", markersize = 5)
