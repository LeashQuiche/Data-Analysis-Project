...
def graphClusters(normArr, centArr, classArr):
    ...
    for i in range(len(centArr)):
        plt.scatter(normArr.h[classArr == i], normArr.g[classArr == i], s = 200, color = colors[i],
                    marker = '.', label = "Class " + str(i))
        plt.scatter(centArr[i, 1], centArr[i, 0], s = 500, color = colors[i], marker = '*', 
