...
def graphCSVFile(normArr):
    ...
    plt.plot(normArr.h[normArr.c == 0], normArr.g[normArr.c == 0], 'b.', label = "No CKD", 
             markersize = 10)
    plt.plot(normArr.h[normArr.c == 1], normArr.g[normArr.c == 1], 'r.', label = "CKD", 
             markersize = 10)
...
def nearestNeighborIndex(normArr, testCase):
    ...
    for i in range(normArr.len):
        distArr[i] = findDistance(normArr.h[i], normArr.g[i], testCase[1], testCase[0])
...
def graphNearestNeighbor(normArr, testCase, testCaseIndex):
    ...
    plt.plot(testCase[1], testCase[0], color[int(normArr.c[testCaseIndex])], label = "Test case", 
             markersize = 15)
