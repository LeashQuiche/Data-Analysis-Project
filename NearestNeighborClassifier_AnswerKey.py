...
def graphCSVFile(normArr):
    ...
    plt.plot(normArr.h[normArr.c == 0], normArr.g[normArr.c == 0], 
             'b.', label = "No CKD", markersize = 10)
    plt.plot(normArr.h[normArr.c == 1], normArr.g[normArr.c == 1], 
...
# Method 1:
# nearestNeighbor takes in a DataArr class and 1D numpy array.
...
def nearestNeighborIndex(normArr, testCase):
    ...
    for i in range(normArr.len):
        distArr[i] = findDistance(normArr.h[i], normArr.g[i],
                                  testCase[1], testCase[0])
    return np.argmin(distArr)

# Method 2: 
# treeNNIndex takes in a DataArr class and 1D numpy array.
...
def graphNearestNeighbor(normArr, testCase, testCaseIndex):
    color = ['bx', 'rx']
    graphCSVFile(normArr)
    plt.plot(testCase[1], testCase[0], color[int(normArr.c[testCaseIndex])], 
             label = "Test case", markersize = 15)
    plt.plot(np.array([testCase[1], normArr.h[testCaseIndex]]),
             np.array([testCase[0], normArr.g[testCaseIndex]]), 'k-')
